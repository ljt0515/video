var VideoTheme = {
	...
	'Mobile': {
			...
			'Init': function () {
				if ($(".nav-slide").length) {
					$(".nav-slide").each(function () {
						var $that = $(this);
						VideoTheme.Mobile.Nav.Set($that, $that.attr('data-align'));
					});
				}
			},
		...
		'Mshare': function () {
			$(".open-share").click(function () {
				VideoTheme.Browser.useragent.weixin ? $("body").append('<div class="mobile-share share-weixin"></div>') : $("body").append('<div class="mobile-share share-other"></div>');
			...
			'Init': function () {
				if ($(".flickity").length) {
					$(".flickity").each(function () {
						var $that = $(this);
						VideoTheme.Images.Flickity.Set($that, $that.attr('data-align'), $that.attr('data-dots'), $that.attr('data-next'), $that.attr('data-play'));
						VideoTheme.Images.Lazyload();
					});
				}
			},
	...
	'Link': {
		'Copy': {
			'Init': function () {
				$(".video-copy-link").each(function () {
					var links = $(this).attr("data-url");
					VideoTheme.Link.Copy.Set(this, links);
				});
				$(".video-copy-html").each(function () {
					var html = $(this).parent().find(".content").html();
					VideoTheme.Link.Copy.Set(this, html);
				});
			},
	...
	'Layer': {
		...
		'Popbody': function (name, title, html, day, wide, high) {
			var pop_is = VideoTheme.Cookie.Get(name);
			var html = $(html).html();
			if (!pop_is) {
				layer.open({
					type: 1,
	...
	'Other': {
		...
		'Sort': function () {
			$(".sort-button").each(function () {
				$(this).on("click", function (e) {
					e.preventDefault();
					$(this).find("i").eq(0).attr("class",$(this).find("i").eq(0).attr("class")==="fa fa-sort-numeric-asc"?"fa fa-sort-numeric-desc":"fa fa-sort-numeric-asc");
					$(this).parent().parent().parent().find(".sort-list").each(function () {
						var playlist = $(this).find("li");
						for (var i = 0, j = playlist.length - 1; i < j;) {
		...
		'Refresh': function () {
			$(".refresh-button").each(function () {
				$(this).on("click", function (e) {
					e.preventDefault();
					$(this).attr("data-id")
					VideoTheme.Ajax( '/video/update/' + $(this).attr('data-id')+".html" , 'get', 'json', '', function (r) {
		...
		'History': {
			'Init': function () {
				if ($(".vod_history").length) {
					...
					VideoTheme.Other.History.Set($that.attr('data-name'), $that.attr('data-link'), $that.attr('data-pic'), $that.attr('data-part'), $that.attr('data-limit'));
				}
			},
			'Set': function (name, link, pic, part, limit) {
				...
				var history = VideoTheme.LocalStorage.Get("history");
				var len = 0;
				var canadd = true;
				if (history) {
					...
					$(history).each(function () {
						if (name === this.name) {
							...
							json += "]";
							VideoTheme.LocalStorage.Set('history', json,7);
							return false;
						}
					});
				}
		...
		'Player': function () {
			if ($("#player-left").length) {
				...
				if (!VideoTheme.Browser.useragent.mobile) {
...
$(function(){
	if(VideoTheme.Browser.useragent.mobile){
		VideoTheme.Mobile.Nav.Init();
		VideoTheme.Mobile.Mshare();
	}
	VideoTheme.Images.Lazyload();
	VideoTheme.Images.Qrcode.Init();
	VideoTheme.Images.Flickity.Init();
	VideoTheme.Link.Copy.Init();
	VideoTheme.Other.Bootstrap();
	VideoTheme.Other.Sort();
	VideoTheme.Other.Refresh();
	VideoTheme.Other.Headroom();
	VideoTheme.Other.Search();
	VideoTheme.Other.Collapse();
	VideoTheme.Other.Slidedown();
	VideoTheme.Other.Scrolltop();
	VideoTheme.Other.History.Init();
	VideoTheme.Other.Player();
	VideoTheme.Other.Close();
});